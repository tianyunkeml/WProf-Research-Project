...
def main():
    ...
    for run_no in range(ITERATION):
        ...
        for site in top_sites:
            s1 = urlparse(site[0])
            my_run = chromium_driver.RunChromium(site[0],'no_cache',net_profile,'dns','record',str(run_no))
            my_run_no_cache = chromium_driver.RunChromium(site[0],'no_cache',net_profile,'','record',str(run_no))
            my_run_cache = chromium_driver.RunChromium(site[0],'cache',net_profile,'','record',str(run_no))
            my_run.main_run()
            my_run_no_cache.main_run()
            my_run_cache.main_run()
        time.sleep(WAIT_TIME - TWO_RUNTIME)
        ...
        for pre_file in pre_files:
            ...
            for net_profile in my_profiles:
                ...
                if conv_orig.do_analysis(net_profile,'cache','',cache_or_not,str(run_no)):
